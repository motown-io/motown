...
public class OcpiRepository {
    ...
    {
        try {
        	return entityManager.createQuery("SELECT endpoint FROM Endpoint AS endpoint", Endpoint.class).getResultList();
    ...
    {
        try {
        	return entityManager.createQuery("SELECT subscription FROM Subscription AS subscription", Subscription.class).getResultList();
    ...
    {
        try {
            return entityManager.createQuery("SELECT subscription FROM Subscription AS subscription WHERE lukasAuthorizationToken = :lukasAuthorizationToken", Subscription.class)
            		.setParameter("lukasAuthorizationToken", lukasAuthorizationToken)
    ...
    {
        try {
            return entityManager.createQuery("SELECT token FROM Token AS token WHERE uid = :uid and issuingCompany = :issuingCompany", Token.class)
            		.setParameter("uid", uid)
            		.setParameter("issuingCompany", issuingCompany)
            		.setMaxResults(1).getSingleResult();
        } catch (NoResultException e){
        	return null;
        } finally {
    ...
    {
        try {
            return entityManager.createQuery("SELECT token FROM Token AS token WHERE visualNumber = :visualNumber", Token.class)
            		.setParameter("visualNumber", visualNumber)
            		.setMaxResults(1).getSingleResult();
        } catch (NoResultException e){
        	return null;
        } finally {
    ...
    public TokenSyncDate getTokenSyncDate() {
        try {
            List<TokenSyncDate> list = entityManager.createQuery("SELECT syncDate FROM TokenSyncDate AS syncDate", TokenSyncDate.class)
