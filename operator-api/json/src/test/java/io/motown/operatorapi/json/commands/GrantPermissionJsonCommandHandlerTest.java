...
public class GrantPermissionJsonCommandHandlerTest {

    private Gson gson;

    private GrantPermissionJsonCommandHandler handler = new GrantPermissionJsonCommandHandler();
    ...
    public void handleGrantPermissionVerifyGatewayCall() {
        ...
        verify(gateway).send(new GrantPermissionCommand(new ChargingStationId(CHARGING_STATION_ID_STRING), new SimpleUserIdentity(user), commandClass, IDENTITY_CONTEXT));
    }
