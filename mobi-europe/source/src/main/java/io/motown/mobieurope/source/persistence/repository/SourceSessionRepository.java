...
public class SourceSessionRepository {
    ...
    public SessionInfo findSessionInfoByRequestId(String requestId) {
        EntityManager entityManager = entityManagerFactory.createEntityManager();

        try {
            return entityManager.createQuery("SELECT si FROM SessionInfo AS si WHERE si.requestIdentifier = :requestIdentifier", SessionInfo.class)
                    .setParameter("requestIdentifier", requestId)
                    .getSingleResult();
        } finally {
            entityManager.close();
        }
